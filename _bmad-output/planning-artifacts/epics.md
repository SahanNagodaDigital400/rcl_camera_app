...
FR-7: Ranked candidate results with images — up to three Candidates ranked by visual similarity, each with reference image, code, size, and category; never a single unverifiable answer; never a similarity number on screen (AD-20); Candidates are distinct Tiles and are never deduplicated or collapsed by Category (AD-18).
...
FR-14: Add tile — Administrator adds a new Tile with a Code and its reference image; searchable immediately, no developer involvement. One catalogue entry per file (AD-18).
FR-15: Edit tile — Administrator changes a Tile's Code and adds/replaces/removes its reference images; a removed image never resurfaces as a Candidate.
FR-16: Remove tile — Administrator removes a Tile from the Catalogue; never returned as a Candidate again (hard delete).
FR-17: Bulk upload — Administrator bulk-loads Tiles via an image set + spreadsheet of codes, with a per-row success/failure report. The realistic path for a new range, since a range is many Tiles.
FR-18: Catalogue search — Administrator searches/filters the Catalogue by Code, including partial matches. (A Staff-facing equivalent on the scan screen is PRD OQ-16, not in scope here.)
...
- 20 binding architecture invariants (AD-1 through AD-20) govern implementation; the dev agent must read `ARCHITECTURE-SPINE.md` directly, not just this summary. Notably: `shared/vision`'s preprocessing/embedding is called identically by the live API and the batch ingest script, never forked (AD-1); crop executes server-side only, client sends the full downscaled image plus normalized coordinates, never a pre-cropped image (AD-11); sessions and rate-limit counters live in Postgres, not Redis (AD-3, AD-8); audit-log immutability is enforced at the DB-role level, not just app code (AD-4); scan history is a denormalized snapshot, never a live foreign key to Reference Image (AD-10); the unit of identity is the Tile — one catalogue file — so nothing deduplicates Candidates by Category and accuracy is scored against the exact file (AD-18).
- **A working POC (`poc/`) validates the core matching approach and pins eight more invariants (AD-13–20), reconciled into the architecture spine on 2026-09-08 and again on 2026-09-17.** The dev agent should port `shared/vision` directly from `poc/tilematch/vision.py` — it was written to lift into production unchanged, not to be reimplemented from. Key points affecting Epic 2/3 stories: each Reference Image needs **up to 16 embeddings** (4 rotations + 12 augmented crops), not one — `REFERENCE_IMAGE` gained a child `REFERENCE_EMBEDDING` entity in the ERD (AD-13); ICC color management is mandatory in `shared/vision` before any other step — ~60% of the real catalogue is CMYK press files, and a naive RGB conversion silently corrupts both the displayed color and the embedding (AD-15); reference images are served from a pre-generated, capped derivative, never the original or an on-demand render (AD-17); embedding inference is serialized server-side, independent of abuse-defense rate limiting — concurrent CPU-bound inference measured 12× slower under load (AD-16); the index carries a pipeline-version stamp and refuses to search a mismatched generation (AD-14). **The 2026-09-17 pass adds three more, and the first of them changes the domain model every story inherits:** the unit of identity is the **Tile — one catalogue file** — so Candidates are never deduplicated or collapsed by Category and accuracy is scored against the exact file (AD-18, and see the corrected PRD §3 Glossary); a staff-declared narrowing attribute, if the product adopts one, is a hard pre-filter applied before ranking rather than a re-rank (AD-19); and no similarity value ever reaches the screen, with any display threshold living in one server-side rule the client paints rather than re-derives (AD-20).
- **Real accuracy data exists, and the honest numbers are lower than the ones earlier revisions of this document quoted.** Current POC figures, scored against the **exact Tile** on a 381-Tile catalogue: top-1 55.6% / top-3 78.7% with no size declared, top-1 59.1% / top-3 81.9% with it declared. Both come from synthetic queries — a degraded copy of an image the index still holds — which is a **loose upper bound**, not an accuracy result, because a warp of image X is far closer to X than any phone photo of the physical tile. ⚠ The often-quoted "79.5% at 36 products dropping to 70.0% at 76" came from the retired leave-one-out harness with loose `size + category` scoring and **cannot be reproduced**; the downward trend with catalogue size is probably real, the magnitude is not evidence. Production targets a catalogue far larger than 381 Tiles. This is now PRD risk R-6 and should set the Epic 3 / Phase 2 pilot's accuracy targets — against real staff photos, since no synthetic mode can substitute for them.
- **Three POC-raised product decisions are open and not scoped into any story below** (PRD OQ-15/16/17): a Size picker on the scan screen before submit (measured +3.2 points of top-3 when correct, and an unreachable true Tile when wrong); a Staff-facing catalogue lookup by Code, for when there is a code but no tile to photograph (FR-18 gives Administrators this already); and expanding the results list beyond three when several Candidates score closely. Each is an addition to FR-7/FR-18's current contract, so each needs a product answer before it becomes a story. The architecture already pins the mechanics of the first and third if they are adopted (AD-19, AD-20) — that is deliberate, so adoption stays a product call rather than an implementation accident.
...
UX-DR19: Microcopy follows `EXPERIENCE.md.Voice and Tone` verbatim where quoted ("Best match" never a percentage; "Fill the frame with the tile face."; plain factual errors; no exclamation marks, no gamified language). Architecture AD-20 hardens the first of these from a tone preference into an invariant, with the measurement behind it: a wrong top-1 answer medians 0.907 against 0.918 for a correct one, so any similarity value on screen — number, bar, or derived wording — is confidence the system does not have.
...
FR-14: Epic 2 - Add tile
FR-15: Epic 2 - Edit tile
FR-16: Epic 2 - Remove tile
...
**And** a row with no recoverable Category name or trailing number is not treated as an error — it's indexed with an explicit unknown marker and flagged in the report for follow-up, since real catalogue data contains both (per the architecture spine's POC-informed data-shape notes)
**And** two rows sharing a Size and Category are both indexed as distinct Tiles — never merged, deduplicated, or reported as a conflict, since the Code is the identity (AD-18).
...
**POC-informed additions (architecture AD-13, AD-14, AD-15, AD-16, AD-18, AD-20):** matching searches multiple embeddings per Reference Image and takes the best score (max-pool), the index refuses to search a stale generation, a Scan's own color profile is corrected the same way a Reference Image's is, and inference is serialized server-side — all implementation mechanics of Story 3.4's "matching completes," not new user-facing stories. Two later additions do reach the surface of Story 3.4 and are written into its ACs: each Candidate is a distinct **Tile**, never collapsed by Category (AD-18), and no similarity value is rendered anywhere on the results screen (AD-20).
...
**Then** I see up to three Candidates ranked by visual similarity, each with its reference image, code, size, and category
**And** each Candidate is a distinct **Tile** — two Candidates from the same Category folder are two different answers and both keep their slot; nothing deduplicates, collapses, or diversifies the list by Category (AD-18)
**And** no similarity value appears anywhere on the screen — not a percentage, a bar, a star rating, or wording derived from it (AD-20); the score stays in the API response and the server log
**And** the result is never a single "confidence-gated" answer, and crop-confirmation-to-result stays under 3 seconds.

**Note (not yet an AC):** whether the list should ever expand beyond three when several Candidates score closely is PRD OQ-17, unanswered. If it is taken up, AD-20 fixes the mechanics — one server-side rule, the client paints the count rather than re-deriving it, runtime-tunable with no re-index, and an empty result renders the empty state with an explicit "show the closest anyway" tap rather than a silent back-fill. The POC's own experience is the caution: at its 0.75 bar the median scan expanded to 19 of 20, long rather than selective. Keep the displayed count and the scored top-3 as separate numbers either way.

**Note (not yet an AC):** PRD OQ-15 asks whether Staff should declare the tile's Size before submitting (+3.2 points of top-3 when right; an unreachable true Tile when wrong). If adopted, it lands on Story 3.1's surface and this story's search path, as a hard pre-filter with an explicit "All sizes" default (AD-19) — not as a re-rank.
