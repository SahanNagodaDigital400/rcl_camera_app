 ...
 * called for the very tile that was handed over. That case has nowhere to stay:
 * the screen was opened about one tile, that tile is gone, and re-rendering as
 * a code-entry stage would leave an Administrator looking at a box for a Code
 ...
 *
 * **The identity of the removed tile decides, not the prop.** `tile` is the
 * mount-time prop and never changes, but the tile on screen does: a `404` on a
 * save or a removal drops back to the lookup stage, from which a *different*
 * tile can be found and removed. Keying the exit on the prop would send that
 * removal to the Catalogue — a surface that never listed it — and swallow its
...
export function EditTileScreen({
  ...
  async function removeTile(): Promise<void> {
    ...
    // And its id, for the same reason one step further: the exit below is only
    // this tile's exit. The screen can be holding a tile it found for itself
    // through the lookup stage after a `404` dropped the one it was handed, and
    // that removal belongs to the lookup, not to the Catalogue.
    const removedId = tile.id;
    ...
    try {
      ...
      setFiles([]);
      if (imagesRef.current) imagesRef.current.value = '';
      if (onRemoved !== undefined && opened?.id === removedId) {
